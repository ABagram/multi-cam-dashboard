...
#include "Arduino.h"
#include <Wifi.h>
#include "esp_heap_caps.h" // Needed for PSRAM/Heap checks
#include "esp_chip_info.h" // Needed for chip info
//#include "driver/temp_sensor.h" // Needed for internal temp sensor

String getSystemDataJson() {
    // 1. Internal Temperature
    float temp_celsius = temperatureRead(); // uses Legacy API
    ...
    // 4. Uptime (milliseconds converted to seconds/minutes/hours)
    unsigned long upTimeMillis = millis();
    unsigned int seconds = upTimeMillis / 1000;
    unsigned int minutes = seconds / 60;
    unsigned int hours = minutes / 60;

    // Build the JSON string
    String json = "{";
    json += "\"temp\":" + String(temp_celsius, 1) + ",";
    json += "\"rssi\":" + String(rssi) + ",";
    json += "\"psram_free\":" + String(psram_free) + ",";
    json += "\"uptime_h\":" + String(hours) + ",";
    json += "\"uptime_m\":" + String(minutes % 60) + ",";
    json += "\"uptime_s\":" + String(seconds % 60);
    // You would add a battery level here if you had ADC code:
    // json += ",\"batt\":3.9"; 
    json += "}";
...
void startCameraServer()
{
    ...
    httpd_uri_t status_uri = {
        .uri = "/status.json", // changed from /status to /status.json
        .method = HTTP_GET,
